    ...
    static async get(id: string): Promise<PageAdver | null> {
        ...
        try {
            const res = await client.query(
                ...
                'WHERE P.id = $3 ',
                [CodeClassDef.CLASS_COLORTYPE, CodeClassDef.CLASS_PAGEADVERSTATUS, id]
            );
            if (res.rowCount < 1) return null;
    ...
    static async selectByPubInfo(publishId: number): Promise<PageAdver[] | null> {
        ...
        try {
            const res = await client.query(
                ...
                'LEFT JOIN t_account_user US ON US.id = P.user_id ' +
                'LEFT JOIN t_config_code_def CC ON CC.class=$2 AND CC.code = P.status ' +
                'WHERE P.publish_id=$3 ORDER BY P.edition_id ',
                [CodeClassDef.CLASS_COLORTYPE, CodeClassDef.CLASS_PAGEADVERSTATUS, publishId]
            );
            let ret = [];
